...
@section('contant')
    <div class="container-fluid">
        ...
        <form action="{{url('apps/update/'.$app->id)}}" method="POST" enctype="multipart/form-data" id="app-data-form">
            @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-2">
                            <div class="form-check">
                                ...
                                @php
                                if($key == 'Google'){
                                $value = \App\Models\Google::where('app_id',$app->id)->first([$ad_type->name,$ad_type->name.'_status']);
                                }
                                if($key == 'Facebook'){
                                ...
                                @endphp
                                @if($key == 'Google')
                                <input type="checkbox" data-toggle="switchbutton" data-onstyle="primary" data-size="sm" class="google-switch" @if($value[$ad_type->name.'_status'] == true) checked @endif data-name="{{$ad_type->name}}" />
                                @endif
...
<script>
    ...
    $('.google-switch').change(function(){
        dataName = $(this).data('name');
        $.ajax({
            url: baseUrl+'/change-google-variable-status/'+dataName+'/'+app_id,
            success: function(resonse){
                $('#Google-ajax_msg').text(resonse.msg)
            },
            error: function(){}
        });
    })
        ...
        $('#app-data-form').submit(function(e){
            e.preventDefault();
            let formData = new FormData(this);
            $.ajax({
            type:'POST',
            url: baseUrl+"/apps/update/"+app_id,
                data: formData,
                contentType: false,
                processData: false,
                success: (response) => {
                if (response) {
                    $('#adsMsg').text(response.msg);
                }
                },
                error: function(response){
                    $('#adsMsg').removeClass('text-success').addClass('text-danger').text(response.responseJSON.message);
                }
            });
        });
    });
